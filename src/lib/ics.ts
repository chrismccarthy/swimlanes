 ...
 *
 * Deliberately dependency-free and side-effect-free: the same source is copied
 * verbatim to `supabase/functions/_shared/ics.ts` so the subscribable feed
 * served by the edge function is byte-for-byte the same as the downloaded file.
 * `src/lib/ics.test.ts` imports both copies and compares their output.
 */
